...
public class BookRentService {
	...
	public static final int BOOK_RENT_RESERVED = 2;

	BookDao bookDao = new BookDaoImpl();
	BookRentDao dao = new BookRentDaoImpl();
	BookReservationDao bookReservationDao = new BookReservationDaoImpl();
	...
	public RentDetail returnOneBook(int rentDetailUid) throws DmlException {
		...
		try {
			...
			boolean isReserved = bookReservationDao.getActiveReservationByBookUid(con, book.getBookUid()) != null;
			if (book != null) {
				if(isReserved) 
					bookDao.updateBookStatus(con, book, BOOK_RENT_RESERVED);
				else

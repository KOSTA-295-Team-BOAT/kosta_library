	...
	public void display(User user) {
		...
		while (true) {
			...
			System.out.println("관심분야: "
					+ (favoriteCategoryNames.isEmpty() ? "설정되지 않음" : String.join(", ", favoriteCategoryNames)));
			System.out.println();
			System.out.println("--------------------------------------------------------------------------------");
			System.out.println();
			System.out.print("1. 이름 변경 | ");
			System.out.print("2. 관심분야 변경 | ");
			System.out.print("3. 관심분야 삭제 | ");
			...
			case 2:
				...
				System.out.println("현재 관심 카테고리 "
						+ (favoriteCategoryNames.isEmpty() ? "없음" : String.join(", ", favoriteCategoryNames)));
				System.out.println();
				System.out.println("--------------------------------------------------------------------------------");
				System.out.println("사용 가능한 카테고리 목록:");
				bookCategoryDao.getAllCategories().forEach(category -> System.out
						.println("ID: " + category.getCategoryUid() + ", 이름: " + category.getCategoryName()));

				// 기존 관심 카테고리 ID 조회
				Set<Integer> existingCategoryIds = new HashSet<>(
						favoriteCategories != null ? favoriteCategories : new ArrayList<>());

				//TODO 여기서 존재하지 않는 카테고리 선택하면 예외 발생함 버그 수정 필요
